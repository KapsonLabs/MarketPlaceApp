...
import { REQUEST_STATUSES, REQUEST_WIZARD_STEPS } from "@/lib/marketplace/request-types";
...
export const listUserRequests = createServerFn({ method: "GET" })
  .validator((input) => userRequestsSchema.parse(input))
  .handler(async ({ data }) => {
...
export const recordPayment = createServerFn({ method: "POST" })
  .validator((input) => paymentSchema.parse(input))
  .handler(async ({ data }) => {
...
export const submitReview = createServerFn({ method: "POST" })
  .validator((input) => reviewSchema.parse(input))
  .handler(async ({ data }) => {
...
export const listReviewsForProvider = createServerFn({ method: "GET" })
  .validator((input) => providerReviewsSchema.parse(input))
  .handler(async ({ data }) => {
